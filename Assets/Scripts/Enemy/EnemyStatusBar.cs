...
public class EnemyStatusBar : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (canvas != null)
        {
            ...
            canvas.worldCamera = CameraCache.Main;
        ...
        if (rt != null)
        {
            ...
            rt.anchoredPosition = Vector2.zero; // 重置 Prefab 里遗留的 (1920,1080) 错误偏移
    ...
    private void LateUpdate()
    {
        ...
        // 相机兜底：开始界面时 MainCamera 可能 inactive，导致 worldCamera 为 null
        if (_canvas != null && _canvas.worldCamera == null)
            _canvas.worldCamera = CameraCache.Main;

        transform.position = _enemy.transform.position + _offset;

        if (CameraCache.Main != null)
        {
            Vector3 dir = CameraCache.Main.transform.position - transform.position;
            dir.y = 0f;
            if (dir != Vector3.zero)
